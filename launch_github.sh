...
do
    ...
    if [ "$experiment" == "CIFAR" ]; then
        ~/mpi_install/bin/mpirun -n $numProcs --rankfile $RANKFILE --mca btl tcp,self,vader --mca pml ob1 -report-bindings --display-map --bind-to core --oversubscribe -quiet python ./eDNNcomm/CIFAR/main.py --manualSeed $seed --bsz 128 --epochs $epochs --mode $mode --worldsize $numProcs --dataset $dt --model $model --lr 0.1 --wd 5e-4 --partition $partition --sharingiter $sharing
    elif [ "$experiment" == "GAN" ]; then
            mpirun -n $numProcs --rankfile $RANKFILE --mca btl tcp,self,vader --mca pml ob1 -report-bindings --display-map --nooversubscribe --mca hwloc_base_binding_policy=core -quiet python ./eDNNcomm/GAN/gan.py --mode $mode --manualSeed 1 --worldsize $numProcs --iteration $i
