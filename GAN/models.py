...
class Generator(nn.Module):
    def __init__(self, n_classes, latent_dim, img_size, channels, k):
        ...
        self.l1 = nn.Sequential(nn.Linear(latent_dim, (128*k) * self.init_size ** 2))

        self.conv_blocks = nn.Sequential(
            nn.BatchNorm2d(128*k),
            nn.Upsample(scale_factor=2),
            nn.Conv2d(128*k, 128*k, 3, stride=1, padding=1),
            nn.BatchNorm2d(128*k, 0.8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Upsample(scale_factor=2),
            nn.Conv2d(128*k, 64*k, 3, stride=1, padding=1),
            nn.BatchNorm2d(64*k, 0.8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64*k, channels, 3, stride=1, padding=1),
            nn.Tanh(),
        )
    ...
    def forward(self, noise, labels):
        ...
        out = out.view(out.shape[0], 128*k, self.init_size, self.init_size)
